                ...
                {isAuthenticated && (
                    <>
                        ...
                        <button 
                            ...
                            className={`btn border-none w-full ${isMinimized ? 'justify-center px-0' : 'justify-start'} ${location.pathname.startsWith('/provas') || location.pathname.startsWith('/campanhas') ? 'bg-[#3B82F6] text-white' : 'btn-ghost text-white/80 hover:bg-[#3B82F6]/50 hover:text-white'}`}
                            title={isMinimized ? "Provas" : undefined}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-6 h-6 ${!isMinimized && 'mr-2'}`}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                            </svg>
                            {!isMinimized && <span>Provas</span>}
                        </button>
                    </>
                )}
            </nav>
